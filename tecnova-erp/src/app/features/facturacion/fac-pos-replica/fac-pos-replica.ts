...
@Component({
  ...
  templateUrl: './fac-pos-replica.html',
  styleUrls: ['./fac-pos-replica.scss']
})
export class FacPosReplicaComponent implements OnDestroy {
  ...
  confirmarCobroLabel = computed(() =>
    this.getCurrentTipoFactura() === 'CCF'
      ...
      .subscribe({
        next: (rows) => {
          this.facturas.set((rows ?? []).filter((item) => {
            ...
            return tipoFactura === 'FAC' || tipoFactura === 'CCF';
          }));
        },
        error: (error) => {
  ...
  openCreate() {
    ...
    this.facForm.reset({
      ...
      RetencionIvaCodigo: '', IvaRetenido: 0, CondicionPago: 'CONTADO', Vendedor: '',
      FormaPagoCodigo: '', FormaPagoMonto: 0, Observaciones: '',
      LineaArticulo: '', LineaDescripcion: '', LineaCantidad: 1, LineaPrecio: 0,
      Sumas: 0,
      Descuentos: 0,
      TotalOperacion: 0,
    ...
    this.tipoFacturaActual.set('FAC');
    this.syncDisabledControls();
  }
  ...
  onClienteSelect(value: string) {
    ...
    this.facForm.patchValue({
      ...
      CorreoElectronico: profile?.CORREO_ELECTRONICO ?? '', Pais: profile?.Pais ?? '',
      Departamento: profile?.Departamento ?? '', Municipio: profile?.Municipio ?? '',
