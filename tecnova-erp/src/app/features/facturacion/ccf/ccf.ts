...
refreshClientes(): void {
  ...
  loadMaestro() {
    ...
    this.facturacionService
      ...
      .subscribe({
        next: (rows) => {
          const facRows = (rows ?? []).filter((item) => (item.Tipo_Factura || '').toUpperCase() === 'CCF');
          this.facturas.set(facRows);
        },
        error: (error) => {
          this.showError('Facturación CCF', this.extractError(error, 'No se pudo cargar el listado de facturas.'));
        }
      });
  }
  ...
  openEdit(item: FacturaGeneralDto) {
    ...
    forkJoin({
      ...
      .subscribe({
        ...
        error: (error) => {
          this.showError('Facturación CCF', this.extractError(error, 'No se pudo cargar el detalle de la factura.'));
        }
      });
  }
  ...
  registrarPago(item: FacturaGeneralDto): void {
    ...
    this.facturacionService.updatePagoRecibido(idFactura, monto, usuario).subscribe({
      ...
      error: (error) => {
        this.showError('Facturación CCF', this.extractError(error, 'No se pudo registrar el pago.'));
      }
    });
  }
  ...
  cancelarGuardarSinCliente() {
    ...
    this.showInfo('Facturación CCF', 'Seleccione un cliente de la lista para continuar.');
  }
  ...
  private executeGuardar() {
    ...
    if (!cliente) {
      this.showError('Facturación CCF', 'Debe seleccionar un cliente antes de guardar.');
      return;
    }
    ...
    this.facturacionService
      ...
      .subscribe({
        next: (response) => {
          ...
          this.hasSavedCurrentRecord.set(true);
          this.showInfo('Facturación CCF', 'Factura guardada correctamente.');
          this.loadMaestro();
        },
        error: (error) => {
          this.showError(
  ...
  aplicar() {
    ...
    if (!idFactura) {
      this.showError('Facturación CCF', 'Primero debe guardar la factura para poder aplicarla.');
      return;
    }
    ...
    this.facturacionService
      ...
      next: () => {
        ...
        this.showInfo('Facturación CCF', 'Factura aplicada correctamente.');
      },
      error: (error) => {
        this.showError('Facturación CCF', this.extractError(error, 'No se pudo aplicar la factura.'));
      }
    });
  }
  ...
  desaplicar() {
    if (!this.canDesaplicar()) {
      this.showError('Facturación CCF', 'Solo las facturas aplicadas sin sello de recepción permiten desaplicar.');
      return;
    }

    const idFactura = this.facForm.controls.IdFactura.value;
    if (!idFactura) {
      this.showError('Facturación CCF', 'No hay factura para desaplicar.');
      return;
    }
    ...
    this.facturacionService.updateFacturacionAplicacion(payload).subscribe({
      next: () => {
        ...
        this.showInfo('Facturación CCF', 'Factura desaplicada correctamente.');
      },
      error: (error) => {
        this.showError('Facturación CCF', this.extractError(error, 'No se pudo desaplicar la factura.'));
      }
    });
  }
  ...
  solicitarEliminarFactura(item: FacturaGeneralDto) {
    if (!this.canEliminarFactura(item)) {
      this.showError('Facturación CCF', 'Solo facturas en elaboración o pendientes de emitir permiten eliminar.');
      return;
    }
  ...
  eliminarFactura(item: FacturaGeneralDto, retryAfterDesaplicar: boolean = true) {
    ...
    const payload: DeleteFacturaDto = {
      ...
      TipoFactura: String(item.Tipo_Factura ?? 'CCF').trim() || 'CCF',
      Fecha: this.resolveFacturaGeneralDate(item.FECHA)
    };

    this.facturacionService.deleteFactura(payload).subscribe({
      next: () => {
        this.showInfo('Facturación CCF', 'Factura eliminada correctamente.');
        this.loadMaestro();
      },
      error: (error) => {
        if (retryAfterDesaplicar && this.shouldRetryDeleteAfterDesaplicar(error)) {
          this.executeDesaplicarForDelete(item).subscribe({
            ...
            error: (desaplicarError) => {
              this.showError('Facturación CCF', this.extractError(desaplicarError, 'No se pudo desaplicar antes de eliminar la factura.'));
            }
          });
          return;
        }

        this.showError('Facturación CCF', this.extractError(error, 'No se pudo eliminar la factura.'));
      }
    });
  }
  ...
  anularDte() {
    if (!this.canAnularDte()) {
      this.showError('Facturación CCF', 'Solo las facturas aplicadas con sello de recepción permiten anular DTE.');
      return;
    }

    const idFactura = this.facForm.controls.IdFactura.value;
    if (!idFactura) {
      this.showError('Facturación CCF', 'No hay factura para anular.');
      return;
    }
  ...
  solicitarConfirmarAnulacion() {
    ...
    if (!motivo) {
      this.showError('Facturación CCF', 'Debe ingresar un motivo de anulación vÃ¡lido.');
      this.focusAnulacionMotivoInput();
      return;
    }
  ...
  confirmarAnulacionDocumento() {
    ...
    const idEmpresa = this.authService.currentUser()?.selectedEmpresa?.idEmpresa ?? 0;
    if (!idEmpresa) {
      this.showError('Facturación CCF', 'No se encontró IdEmpresa en la sesión.');
      return;
    }
    ...
    if (!apiBaseUrl) {
      this.showError('Facturación CCF', 'No se encontró UrlAPI en la sesión de empresa.');
      return;
    }
    ...
    if (!duiSolicita) {
      this.showError('Facturación CCF', 'No se encontró DUI del usuario para anular el DTE. Inicie sesión nuevamente.');
      return;
    }
    ...
    const tipoFactura = String(this.selectedFactura()?.Tipo_Factura ?? 'CCF').trim() || 'CCF';
    ...
    this.facturacionService
          ...
          this.facturacionService
            ...
            .pipe(
              catchError((correoError) => {
                this.showError('Facturación CCF', this.extractError(correoError, 'Documento anulado, pero falló el envÃ­o de correo de notificación.'));
                return of(null);
              })
            )
        ),
      ...
      .subscribe({
        next: () => {
          this.showInfo('Facturación CCF', 'Documento anulado correctamente.');
          this.refreshEncabezadoAfterEmission();
          this.loadMaestro();
        },
        error: (error) => {
          this.showError('Facturación CCF', this.extractError(error, 'No se pudo anular el DTE.'));
        }
      });
  }
  ...
  vistaPrevia() {
    ...
    if (!this.canVistaPrevia()) {
      this.showError('Facturación CCF', 'No se permite vista previa/imprimir para documentos anulados.');
      return;
    }

    const idFactura = this.toNumber(this.facForm.controls.IdFactura.value);
    if (!idFactura) {
      this.showError('Facturación CCF', 'Guarde la factura antes de abrir la vista previa.');
      return;
    }
    ...
    if (!popup) {
      this.showError('Facturación CCF', 'El navegador bloqueó la vista previa. AsegÃºrese de permitir ventanas emergentes.');
    }
  }
  ...
  emitirDte() {
    ...
    if (!this.canEmit()) {
      this.showError('Facturación CCF', 'Solo las facturas aplicadas sin sello de recepción permiten emitir DTE.');
      return;
    }

    const idFactura = this.facForm.controls.IdFactura.value;
    if (!idFactura) {
      this.showError('Facturación CCF', 'Debe guardar la factura antes de emitir DTE.');
      return;
    }
    ...
    this.facturacionService
      ...
      .pipe(
        switchMap((status) => {
          ...
          this.setStep('service', 'ok', 'Servicio en lÃ­nea');
          ...
          return this.refreshEncabezadoAfterEmission(false).pipe(
            switchMap((encabezado) => {
              if ((encabezado.NoControl || '').trim() || (encabezado.SelloRecepcion || '').trim()) {
                ...
                this.showInfo('Facturación CCF', 'La factura ya estaba emitida y se sincronizó su información.');
                return of(null);
              }
        ...
        switchMap((response) => {
          ...
          this.setStep('sync', 'running', 'Recuperando sello y nÃºmero de control');
          return this.refreshEncabezadoAfterEmission(false).pipe(
              switchMap(() => {
                this.setStep('sync', 'ok', 'Datos actualizados');
                this.setStep('correo', 'running', 'Enviando correo y obteniendo formato visual');
                return this.facturacionService.enviarCorreoDte(idEmpresa, idFactura, "CCF").pipe(
      ...
      .subscribe({
        next: (result) => {
          if (result?.emitido) {
            ...
            this.showInfo('Facturación CCF', 'DTE emitido correctamente.');
          }
          this.loadMaestro();
        },
        error: (error) => {
          this.setStep('emit', 'error', this.extractError(error, 'Falló la emisión del DTE.'));
          this.showError('Facturación CCF', this.extractError(error, 'Falló la emisión del DTE.'));
        }
      });
  }
  ...
  agregarFormaPago() {
    ...
    if (this.isNewUnsaved()) {
      this.showError('Facturación CCF', 'Guarde el encabezado antes de agregar formas de pago.');
      return;
    }
    ...
    if (!codigo) {
      this.showError('Facturación CCF', 'Debe seleccionar una forma de pago.');
      return;
    }

    if (monto <= 0) {
      this.showError('Facturación CCF', 'El monto de la forma de pago debe ser mayor a cero.');
      return;
    }

    if (idFactura <= 0) {
      this.showError('Facturación CCF', 'No se pudo obtener IdFactura. Guarde nuevamente e intente agregar la forma de pago.');
      return;
    }
    ...
    if (exists) {
      this.showError('Facturación CCF', `La forma de pago ${descripcion} ya estÃ¡ registrada en esta factura.`);
      return;
    }
  ...
  eliminarFormaPago(index: number) {
    ...
    this.facturacionService.updateFacturaFormaPago(payload).subscribe({
      ...
      error: (error) => {
        this.showError('Facturación CCF', this.extractError(error, 'No se pudo eliminar la forma de pago.'));
      }
    });
  }
  ...
  agregarDetalleManual(skipAutoSave: boolean = false) {
    ...
    if (!articulo) {
      this.showError('Facturación CCF', 'Debe seleccionar o ingresar un articulo.');
      return;
    }

    if (!descripcion) {
      this.showError('Facturación CCF', 'Debe ingresar una descripcion para la linea.');
      return;
    }

    if (cantidad <= 0 || precio < 0) {
      this.showError('Facturación CCF', 'Cantidad y precio deben ser valores validos.');
      return;
    }

    if (precio === 0) {
      this.showError('Facturación CCF', 'No se puede agregar una lÃ­nea con precio unitario en cero.');
      return;
    }

    const codGeneracion = String(this.facForm.controls.CodGeneracion.value ?? '').trim();
    if (!codGeneracion) {
      this.showError('Facturación CCF', 'No se encontró código de generación para registrar el detalle.');
      return;
    }
    ...
    this.facturacionService.updateDetalleFactura(payload).subscribe({
      ...
      error: (error) => {
        this.showError('Facturación CCF', this.extractError(error, 'No se pudo registrar el detalle de factura.'));
      }
    });
  }
...
this.clientesFiltradosParaTabla.set(profiles);
  ...
  private loadFacturaFormaPago(idFactura: number) {
    ...
    this.facturacionService.getFacturaFormaPago(idFactura).subscribe({
      ...
      error: (error) => {
        this.formasPagoDetalle.set([]);
        this.showError('Facturación CCF', this.extractError(error, 'No se pudieron cargar las formas de pago de la factura.'));
      }
    });
  }
  ...
  onIncluyeIvaChange(checked: boolean): void {
    ...
    this.facturacionService.updatePrecioConIva(idFactura, checked).subscribe({
      error: (error) => {
        this.showError('Facturación CCF', this.extractError(error, 'No se pudo actualizar PrecioConIVA.'));
      }
    });
  }
  ...
  private updateFacturaRetencion(retencionCodigo: string) {
    ...
    this.facturacionService.updateFacturaRetencion(payload).subscribe({
      ...
      error: (error) => {
        this.showError('Facturación CCF', this.extractError(error, 'No se pudo actualizar la retención.'));
      }
    });
  }
  ...
  private openDteVisualPreview(htmlCompleto: string) {
    ...
    if (!popup) {
      this.showError('Facturación CCF', 'El navegador bloqueó la vista visual del DTE.');
      return;
    }
  ...
  private numberToSimpleWords(value: number): string {
    ...
    return `${integerPart} CON ${String(decimalPart).padStart(2, '0')}/100 DÃ“LARES`;
  }
}
