...
export class FexComponent {
  ...
  onReenviarCorreo(item: FacturaGeneralDto): void {
    ...
    this.reenviarCorreoDialog.abrir(idEmpresa, idFactura, 'FEX', correoDefault);
  }
        ...
        switchMap((status) => {
          ...
          return this.refreshEncabezado(false).pipe(
            switchMap((encabezado) => {
              ...
              return this.facturacionService.emitirDte(apiBaseUrl, payload, 'FEX');
            })
          );
        }),
        switchMap((response) => {
  ...
  private refreshDetalleYTotales() {
    ...
    this.loadingDetail.set(true);
    this.facturacionService.getFacturaDetalle(prefijo, factura, sucursal, puntoVenta, 'FEX')
      .pipe(finalize(() => {
      ...
      .subscribe({
        ...
        error: (error) => {
          this.showError('Facturación FEX', this.extractError(error, 'No se pudo recargar el detalle actualizado.'));
        }
      });
  }
