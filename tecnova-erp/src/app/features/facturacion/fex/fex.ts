...
@Component({
  ...
  templateUrl: './fex.html',
  styleUrls: ['../fac/fac.scss']
})
export class FexComponent {
  ...
  facJsonPreview = computed(() => {
    ...
    return {
      ...
      receptor: {
        ...
        nrc: null,
        nombre: raw.FacturarA || raw.Nombre || null,
        codActividad: null,
        descActividad: null,
        direccion: {
          departamento: null,
      ...
      cuerpoDocumento: detalle.map((item, index) => ({
        ...
        ventaNoSuj: 0,
        ventaExenta: this.toNumber(item.TotalVenta || item.TOTAL),
        ventaGravada: 0,
        tributos: null,
        psv: 0,
        noGravado: 0,
      ...
      resumen: {
        totalNoSuj: 0,
        totalExenta: this.toNumber(raw.Sumas),
        totalGravada: 0,
  ...
  onRangeChange(field: 'desde' | 'hasta', value: string, eventTarget?: EventTarget | null) {
    ...
    if (!this.isValidFilterDate(desde) || !this.isValidFilterDate(hasta)) return;
  ...
  onSucursalChange(value: string) {
    ...
    if (!isValidPunto) this.facForm.patchValue({ PuntoVenta: '' });
  }
  ...
  loadMaestro() {
    ...
    this.facturacionService
      ...
      .subscribe({
        next: (rows) => {
          const fexRows = (rows ?? []).filter((item) => (item.Tipo_Factura || '').toUpperCase() === 'FEX');
          this.facturas.set(fexRows);
        },
        error: (error) => {
          this.showError('Facturación FEX', this.extractError(error, 'No se pudo cargar el listado de facturas.'));
        }
      });
  }
  ...
  openEdit(item: FacturaGeneralDto) {
    ...
    forkJoin({
      ...
      .subscribe({
        ...
        error: (error) => {
          this.showError('Facturación FEX', this.extractError(error, 'No se pudo cargar el detalle de la factura.'));
        }
      });
  }
  ...
  registrarPago(item: FacturaGeneralDto): void {
    ...
    this.facturacionService.updatePagoRecibido(idFactura, monto, usuario).subscribe({
      next: () => { this.loadMaestro(); },
      error: (error) => {
        this.showError('Facturación FEX', this.extractError(error, 'No se pudo registrar el pago.'));
      }
    });
  }
  ...
  guardar() {
    if (!this.canSave()) {
      this.showError('Facturación FEX', 'Solo las facturas en elaboración permiten guardar cambios.');
      return;
    }
  ...
  aplicar() {
    if (!this.canApply()) {
      this.showError('Facturación FEX', 'Solo las facturas en elaboración permiten aplicar.');
      return;
    }
    ...
    const payload: UpdateFacturacionAplicacionDto = {
      ...
      TipoFactura: String(this.selectedFactura()?.Tipo_Factura ?? 'FEX').trim() || 'FEX',
    ...
    this.facturacionService.updateFacturacionAplicacion(payload).subscribe({
      next: () => {
        ...
        this.showInfo('Facturación FEX', 'Factura aplicada correctamente.');
      },
      error: (error) => {
  ...
  desaplicar() {
    if (!this.canDesaplicar()) {
      this.showError('Facturación FEX', 'Solo las facturas aplicadas sin sello de recepción permiten desaplicar.');
      return;
    }

    const idFactura = this.facForm.controls.IdFactura.value;
    if (!idFactura) {
      this.showError('Facturación FEX', 'No hay factura para desaplicar.');
      return;
    }
    ...
    this.facturacionService.updateFacturacionAplicacion(payload).subscribe({
      next: () => {
        ...
        this.showInfo('Facturación FEX', 'Factura desaplicada correctamente.');
      },
      error: (error) => {
        this.showError('Facturación FEX', this.extractError(error, 'No se pudo desaplicar la factura.'));
      }
    });
  }
  ...
  solicitarEliminarFactura(item: FacturaGeneralDto) {
    if (!this.canEliminarFactura(item)) {
      this.showError('Facturación FEX', 'Solo facturas en elaboración o pendientes de emitir permiten eliminar.');
      return;
    }
  ...
  confirmarEliminarFactura() {
    ...
    this.confirmDeleteFacturaDialogVisible.set(false);
    if (!target) {
      this.showError('Facturación FEX', 'No hay factura seleccionada para eliminar.');
      return;
    }
  ...
  eliminarFactura(item: FacturaGeneralDto, retryAfterDesaplicar: boolean = true) {
    ...
    const payload: DeleteFacturaDto = {
      ...
      TipoFactura: String(item.Tipo_Factura ?? 'FEX').trim() || 'FEX',
      Fecha: this.resolveFacturaGeneralDate(item.FECHA)
    };

    this.facturacionService.deleteFactura(payload).subscribe({
      next: () => {
        this.showInfo('Facturación FEX', 'Factura eliminada correctamente.');
        this.loadMaestro();
      },
      error: (error) => {
        if (retryAfterDesaplicar && this.shouldRetryDeleteAfterDesaplicar(error)) {
          this.executeDesaplicarForDelete(item).subscribe({
            next: () => { this.eliminarFactura(item, false); },
            error: (desaplicarError) => {
              this.showError('Facturación FEX', this.extractError(desaplicarError, 'No se pudo desaplicar antes de eliminar la factura.'));
            }
          });
          return;
        }
        this.showError('Facturación FEX', this.extractError(error, 'No se pudo eliminar la factura.'));
      }
    });
  }
  ...
  anularDte() {
    if (!this.canAnularDte()) {
      this.showError('Facturación FEX', 'Solo las facturas aplicadas con sello de recepción permiten anular DTE.');
      return;
    }

    const idFactura = this.facForm.controls.IdFactura.value;
    if (!idFactura) {
      this.showError('Facturación FEX', 'No hay factura para anular.');
      return;
    }
  ...
  solicitarConfirmarAnulacion() {
    ...
    if (!motivo) {
      this.showError('Facturación FEX', 'Debe ingresar un motivo de anulación válido.');
      this.focusAnulacionMotivoInput();
      return;
    }
  ...
  confirmarAnulacionDocumento() {
    ...
    const idEmpresa = this.authService.currentUser()?.selectedEmpresa?.idEmpresa ?? 0;
    if (!idEmpresa) {
      this.showError('Facturación FEX', 'No se encontró IdEmpresa en la sesión.');
      return;
    }
    ...
    if (!apiBaseUrl) {
      this.showError('Facturación FEX', 'No se encontró UrlAPI en la sesión de empresa.');
      return;
    }
    ...
    if (!duiSolicita) {
      this.showError('Facturación FEX', 'No se encontró DUI del usuario para anular el DTE. Inicie sesión nuevamente.');
      return;
    }
    ...
    const tipoFactura = String(this.selectedFactura()?.Tipo_Factura ?? 'FEX').trim() || 'FEX';
    ...
    this.facturacionService
      ...
      .pipe(
        switchMap((respuesta) => {
          ...
          const yaInvalidado = mensajeGeneral.toUpperCase().includes('DOCUMENTO SE ENCUENTRA INVALIDADO');
          if (!yaProcesado && !yaInvalidado) {
          ...
          this.facturacionService
            ...
            .pipe(
              catchError((correoError) => {
                this.showError('Facturación FEX', this.extractError(correoError, 'Documento anulado, pero falló el envío de correo.'));
                return of(null);
              })
            )
        ),
      ...
      .subscribe({
        ...
        error: (error) => {
          this.showError('Facturación FEX', this.extractError(error, 'No se pudo anular el DTE.'));
        }
      });
  }
  ...
  private focusAnulacionMotivoInput() {
    ...
    requestAnimationFrame(() => {
      const input = document.getElementById('fex-anulacion-motivo') as HTMLInputElement | null;
  ...
  vistaPrevia() {
    ...
    if (!this.canVistaPrevia()) {
      this.showError('Facturación FEX', 'No se permite vista previa/imprimir para documentos anulados.');
      return;
    }

    const idFactura = this.toNumber(this.facForm.controls.IdFactura.value);
    if (!idFactura) {
      this.showError('Facturación FEX', 'Guarde la factura antes de abrir la vista previa.');
      return;
    }
    ...
    if (!popup) {
      this.showError('Facturación FEX', 'El navegador bloqueó la vista previa. Permita ventanas emergentes.');
    }
  }
  ...
  emitirDte() {
    ...
    const idFactura = this.facForm.controls.IdFactura.value;
    if (!idFactura) {
      this.showError('Facturación FEX', 'Debe guardar la factura antes de emitir DTE.');
      return;
    }
          ...
          return this.refreshEncabezadoAfterEmission(false).pipe(
            switchMap((encabezado) => {
              if ((encabezado.NoControl || '').trim() || (encabezado.SelloRecepcion || '').trim()) {
                ...
                this.showInfo('Facturación FEX', 'La factura ya estaba emitida y se sincronizó su información.');
                return of(null);
              }
        ...
        switchMap((response) => {
          ...
          return this.refreshEncabezadoAfterEmission(false).pipe(
            switchMap(() => {
              ...
              return this.facturacionService.enviarCorreoDte(idEmpresa, idFactura, 'FEX').pipe(
      ...
      .subscribe({
        next: (result) => {
          if (result?.emitido) {
            ...
            this.showInfo('Facturación FEX', 'DTE emitido correctamente.');
          }
          this.loadMaestro();
        },
        error: (error) => {
          this.setStep('emit', 'error', this.extractError(error, 'Falló la emisión del DTE.'));
          this.showError('Facturación FEX', this.extractError(error, 'Falló la emisión del DTE.'));
        }
      });
  }
  ...
  estadoVisualKey(estado: unknown, selloRecepcion: unknown): 'ELABORACION' | 'PENDIENTE_EMITIR' | 'EMITIDO' | 'ANULADO' | 'OTRO' {
    ...
    if (normalized === 'ELABORACION') return 'ELABORACION';
    return 'OTRO';
  }

  estadoVisualLabel(estado: unknown, selloRecepcion: unknown): string {
    const visual = this.estadoVisualKey(estado, selloRecepcion);
    if (visual === 'PENDIENTE_EMITIR') return 'PENDIENTE DE EMITIR';
    if (visual === 'EMITIDO') return 'EMITIDO';
    return visual;
  }
  ...
  canVistaPrevia(): boolean {
    const visual = this.estadoVisualKey(this.facForm.controls.Estado.value, this.facForm.controls.SelloRecepcion.value);
  ...
  onArticuloSelect(displayValue: string) {
    ...
    if (!articulo) {
      this.facForm.patchValue({ LineaArticulo: '', LineaDescripcion: '' });
      return;
    }
    ...
    this.facturacionService
      ...
      .subscribe({
        next: (rows) => {
          ...
          this.retencionAplicada.set({ codigo: selected.codigo, descripcion: selected.descripcion, monto: this.toNumber(selected.monto) });
          this.applyRetencionMonto(this.toNumber(selected.monto));
        },
        error: () => { this.retencionAplicada.set(null); }
      });
  }
  ...
  private refreshRetencionAplicadaFromBackend() {
    ...
    this.facturacionService
      .getFacturaRetenciones(context.cliente, context.prefijo, context.factura, context.sucursal, context.puntoVenta, 'IVA')
      .subscribe({
        next: (rows) => {
          const mapped = (rows ?? [])
            .map((row: FacturaRetencionDto) => ({
  ...
  private getRetencionQueryContext() {
    const raw = this.facForm.getRawValue();
  ...
  private applyRetencionMonto(_retencionMonto: number) {
    const idFactura = this.toNumber(this.facForm.controls.IdFactura.value);
    if (idFactura > 0) {
  ...
  agregarFormaPago() {
    ...
    if (this.isNewUnsaved()) {
      this.showError('Facturación FEX', 'Guarde el encabezado antes de agregar formas de pago.');
      return;
    }
    ...
    if (!codigo) { this.showError('Facturación FEX', 'Debe seleccionar una forma de pago.'); return; }
    if (monto <= 0) { this.showError('Facturación FEX', 'El monto de la forma de pago debe ser mayor a cero.'); return; }
    if (idFactura <= 0) { this.showError('Facturación FEX', 'No se pudo obtener IdFactura. Guarde nuevamente e intente.'); return; }
    ...
    if (exists) {
      this.showError('Facturación FEX', `La forma de pago ${descripcion} ya está registrada en esta factura.`);
      return;
    }

    const payload: UpdateFacturaFormaPagoDto = { IdFactura: idFactura, Codigo: codigo, Monto: monto, TipoMtto: 'A' };
  ...
  eliminarFormaPago(index: number) {
    ...
    this.facturacionService.updateFacturaFormaPago(payload).subscribe({
      next: () => { this.loadFacturaFormaPago(idFactura); },
      error: (error) => { this.showError('Facturación FEX', this.extractError(error, 'No se pudo eliminar la forma de pago.')); }
    });
  }
  ...
  agregarDetalleManual(skipAutoSave: boolean = false) {
    if (this.isReadOnlyField() || this.saving()) return;
    ...
    if (!articulo) { this.showError('Facturación FEX', 'Debe seleccionar o ingresar un articulo.'); return; }
    if (!descripcion) { this.showError('Facturación FEX', 'Debe ingresar una descripcion para la linea.'); return; }
    if (cantidad <= 0 || precio < 0) { this.showError('Facturación FEX', 'Cantidad y precio deben ser valores validos.'); return; }
    if (precio === 0) { this.showError('Facturación FEX', 'No se puede agregar una línea con precio unitario en cero.'); return; }

    const codGeneracion = String(this.facForm.controls.CodGeneracion.value ?? '').trim();
    if (!codGeneracion) { this.showError('Facturación FEX', 'No se encontró código de generación para registrar el detalle.'); return; }
    ...
    // FEX: el precio viaja tal como el usuario lo ingresa; la API maneja los cálculos
    ...
    this.facturacionService.updateDetalleFactura(payload).subscribe({
      next: () => {
        const selected = this.selectedFactura();
        if (selected?.Prefijo && selected?.Factura) {
          const sucursal = String(selected.CODIGOSUCURSAL || selected.SUCURSAL || '').trim();
          const puntoVenta = String(selected.PUNTO_VENTA || '').trim();
          this.facturacionService
            .getFacturaDetalle(selected.Prefijo, selected.Factura, sucursal, puntoVenta, 'FEX')
            .subscribe({ next: (rows) => { this.detalleRows.set(rows); this.cdr.markForCheck(); } });
        } else {
      ...
      error: (error) => { this.showError('Facturación FEX', this.extractError(error, 'No se pudo registrar el detalle de factura.')); }
    });
  }

  private guardarEncabezadoConSrAParaAgregarDetalle() {
    if (this.saving()) return;
    if (this.requiresClienteConfirmation()) this.applyDefaultSrCliente();
    const payload = this.buildUpdateFacturaPayload();
    ...
    this.facturacionService
      ...
      .subscribe({
        next: (response) => {
          ...
          this.hasSavedCurrentRecord.set(true);
          this.showInfo('Facturación FEX', 'Factura guardada.');
          this.agregarDetalleManual(true);
        },
        error: (error) => {
          this.showError('Facturación FEX', this.extractError(error, 'No se pudo guardar el encabezado antes de agregar el detalle.'));
        }
      });
  }
  ...
  eliminarDetalle(linea: number) {
    ...
    this.facturacionService.updateDetalleFactura(payload).subscribe({
      ...
      error: (error) => { this.showError('Facturación FEX', this.extractError(error, 'No se pudo eliminar el detalle de factura.')); }
    });
  }
    ...
    this.facturacionService.getPerfilClientes().subscribe({
      next: (rows) => {
        ...
        const clientes = Array.from(new Set(
          profiles.map((item) => String(item.NOMBRE ?? '').trim()).filter((name) => !!name)
  ...
  constructor() {
    ...
    this.facturacionService.getCatalogoRetenciones().subscribe({
      next: (rows) => {
        this.retencionesOptions.set((rows ?? []).filter((item) => !!String(item.RETENCION ?? '').trim()));
      },
  ...
  private loadFacturaFormaPago(idFactura: number) {
    if (idFactura <= 0) { this.formasPagoDetalle.set([]); return; }
    ...
    this.facturacionService.getFacturaFormaPago(idFactura).subscribe({
      ...
      error: (error) => {
        this.formasPagoDetalle.set([]);
        this.showError('Facturación FEX', this.extractError(error, 'No se pudieron cargar las formas de pago.'));
      }
    });
  }
  ...
  private refreshEncabezadoAfterEmission(_showToastOnError: boolean = true): Observable<FacturaEncabezadoDto> {
    ...
    return this.facturacionService.getFacturaEncabezado(prefijo, factura, sucursal, punto, idEmpresa).pipe(
      map((encabezado) => { this.patchEncabezado(encabezado); return encabezado; }),
  ...
  onIncluyeIvaChange(checked: boolean): void {
    ...
    this.facturacionService.updatePrecioConIva(idFactura, checked).subscribe({
      error: (error) => { this.showError('Facturación FEX', this.extractError(error, 'No se pudo actualizar PrecioConIVA.')); }
    });
  }
  ...
  private updateFacturaRetencion(retencionCodigo: string) {
    ...
    this.facturacionService.updateFacturaRetencion(payload).subscribe({
      next: () => { this.refreshRetencionAplicadaFromBackend(); },
      error: (error) => { this.showError('Facturación FEX', this.extractError(error, 'No se pudo actualizar la retención.')); }
    });
  }
  ...
  resolveCondicionPagoLabel(codigo: string | null): string {
    ...
    const item = this.condicionesPagoOptions().find((c) => String(c.CONDICION_PAGO ?? '').trim() === code);
  ...
  private validarFormasPago(): boolean {
    ...
    if (!pagos.length) {
      this.showError('Facturación FEX', 'Debe agregar al menos una forma de pago.');
      return false;
    }
    ...
    if (diferencia > 0.009) {
      this.showError('Facturación FEX', `La suma de formas de pago (${sumaPagos.toFixed(2)}) debe ser igual al total a pagar (${totalPagar.toFixed(2)}).`);
      return false;
    }
  ...
  private openDteVisualPreview(htmlCompleto: string) {
    ...
    if (!popup) {
      this.showError('Facturación FEX', 'El navegador bloqueó la vista visual del DTE.');
      return;
    }
  ...
  private extractError(error: unknown, fallback: string): string {
    ...
    if (fromError && typeof fromError === 'object') {
      ...
      for (const candidate of [errorObject['message'], errorObject['detail'], errorObject['title']]) {
  ...
  private sanitizeBackendErrorMessage(message: string): string {
    ...
    if (/The conversion of a nvarchar data type to a datetime data type resulted in an out-of-range value\.?/i.test(normalized)) return '';
    const sqlMatch = normalized.match(/SqlException\s*\([^)]*\)\s*:\s*([^\n]+)/i);
    if (sqlMatch?.[1]) {
      const sqlMessage = String(sqlMatch[1]).trim();
      if (/The conversion of a nvarchar data type to a datetime data type resulted in an out-of-range value\.?/i.test(sqlMessage)) return '';
      return sqlMessage;
    }
  ...
  private resolveFacturaGeneralDate(value: unknown): string {
    ...
    if (!raw) return this.formatDateInput(new Date());
    const datePart = raw.split(' ')[0];
    const normalized = this.normalizeFilterDate(datePart);
    return normalized ?? this.formatDateInput(raw);
  }
  ...
  private normalizeFilterDate(value: unknown): string | null {
    ...
    if (isoMatch) return this.buildValidatedIsoDate(isoMatch[1], isoMatch[2], isoMatch[3]);
    const slashMatch = text.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
    if (slashMatch) return this.buildValidatedIsoDate(slashMatch[3], slashMatch[2], slashMatch[1]);
    return null;
  }
  ...
  private buildValidatedIsoDate(yearText: string, monthText: string, dayText: string): string | null {
    ...
    if (Number.isNaN(candidate.getTime()) || candidate.getFullYear() !== year || candidate.getMonth() !== month - 1 || candidate.getDate() !== day) return null;
  ...
  private patchIdFacturaFromUpdateResponse(response: unknown) {
    ...
    if (idFactura > 0) this.facForm.patchValue({ IdFactura: idFactura });
  }
  ...
  private syncTotalsFromDetalle() {
    ...
    // Sin idFactura (registro nuevo): solo suma bruta de líneas, sin cálculos de IVA
    const total = this.roundAmount(
      this.detalleRows().reduce((acc, row) => acc + this.toNumber(row.TotalVenta || row.TOTAL), 0)
    );
    this.facForm.patchValue({
      Sumas: total,
      Descuentos: 0,
  ...
  private computeTotalsFromDetalleRows(
    ...
    _retencionCode?: string | null,
    _retencionMonto?: number
    ...
    // FEX: sin cálculo de IVA en frontend. La API calcula y devuelve los totales reales.
    // Este método solo se usa para construir el payload al agregar/eliminar líneas.
    const total = this.roundAmount(
      rows.reduce((acc, row) => acc + this.toNumber(row.TotalVenta || row.TOTAL), 0)
    );
    return { sumas: total, descuentos: 0, totalOperacion: total, iva: 0, subTotalVentas: total, retencion: 0, totalFactura: total };
  }

  private refreshFacturaTotales() {
    const idFactura = this.toNumber(this.facForm.controls.IdFactura.value);
    if (idFactura <= 0) return;
    if (this.facturaTotalesRequestInFlightForId === idFactura) return;
    ...
    this.facturacionService.getFacturaTotales(idFactura).pipe(
      finalize(() => {
        if (this.facturaTotalesRequestInFlightForId === idFactura) this.facturaTotalesRequestInFlightForId = null;
        this.loadingTotals.set(false);
        this.cdr.markForCheck();
      })
    ).subscribe({
      next: (totales) => { this.patchFacturaTotales(totales); },
      error: () => {
  ...
  private normalizeFacturaTotals(source: {
    sumas: number; descuentos: number; totalOperacion: number; iva: number;
    subTotalVentas: number; retencion: number; totalFactura: number;
  }) {
    ...
    const totalOperacion = totalOperacionBase > 0 || sumas === 0 ? totalOperacionBase : this.roundAmount(sumas - descuentos);
    const iva = this.roundAmount(source.iva);
    const subTotalVentasBase = this.roundAmount(source.subTotalVentas);
    const subTotalVentas = subTotalVentasBase > 0 || (totalOperacion === 0 && iva === 0) ? subTotalVentasBase : this.roundAmount(totalOperacion + iva);
    const retencion = this.roundAmount(source.retencion);
    const totalFacturaBase = this.roundAmount(source.totalFactura);
    const totalFactura = totalFacturaBase > 0 || (subTotalVentas === 0 && retencion === 0) ? totalFacturaBase : this.roundAmount(subTotalVentas - retencion);
